...
describe('Class unit tests', () => {
  ...
  describe('QUERY_MEMBERS', () => {
    it('provides access to query measures and dimensions', async () => {
      const compilers = prepareYamlCompiler(
        createSchemaYaml({
          cubes: [{
            name: 'TestCube',
      ...
      await compilers.compiler.compile();
      const query = new BaseQuery(compilers, {
        measures: ['TestCube.count', 'TestCube.total'],
        dimensions: ['TestCube.status'],
      });

      const contextSymbols = query.parametrizedContextSymbols();
      expect(contextSymbols.queryMembers).toBeDefined();
      expect(contextSymbols.queryMembers.measures).toEqual(['TestCube.count', 'TestCube.total']);
      expect(contextSymbols.queryMembers.dimensions).toEqual(['TestCube.status']);
      expect(contextSymbols.queryMembers.all).toEqual(['TestCube.count', 'TestCube.total', 'TestCube.status']);
    });

    it('returns empty arrays when no measures or dimensions', async () => {
      const compilers = prepareYamlCompiler(
        createSchemaYaml({
          cubes: [{
            name: 'TestCube',
      ...
      const contextSymbols = query.parametrizedContextSymbols();
      expect(contextSymbols.queryMembers.measures).toEqual([]);
      expect(contextSymbols.queryMembers.dimensions).toEqual([]);
